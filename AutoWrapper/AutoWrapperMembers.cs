    ...
    internal class AutoWrapperMembers
    {
        ...
        public async Task HandleSuccessfulRequestAsync(HttpContext context, object body, int httpStatusCode)
        {
            ...
            if (type.Equals(typeof(JObject)))
            {
                ...
                if (apiResponse.ResponseCode == 0 && apiResponse.Data == null && apiResponse.ResponseException == null)
                    jsonString = ConvertToJSONString(httpStatusCode, bodyContent, context.Request.Method);
                else if ((apiResponse.ResponseCode != httpStatusCode || apiResponse.Data != null) ||
                        (apiResponse.ResponseCode == httpStatusCode && apiResponse.Data == null))
                {
                    httpStatusCode = apiResponse.ResponseCode; // in case response is not 200 (e.g 201)
        ...
        {
            apiResponse.ResponseCode = !_options.ShowStatusCode ? 0 : apiResponse.ResponseCode;
            apiResponse.IsError = SetIsErrorValue(apiResponse.IsError);
            return JsonConvert.SerializeObject(apiResponse, _jsonSettings);
        }
        ...
        {
            apiResponse.Data ??= $"{httpMethod} {ResponseMessage.Success}";
